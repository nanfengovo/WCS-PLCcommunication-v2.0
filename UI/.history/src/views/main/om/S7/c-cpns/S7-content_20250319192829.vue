<template>
    <div class="s7-content" v-if="isMounted" v-loading="loading">
        ...
        <!-- 分页 -->
        <!-- 分页控件 -->
        <el-pagination background layout="prev, pager, next" :total="total" :currentpage="page" :page-size="limit"
...
const saveS7Config = async (row: any) => {
    await axios.put(`http://localhost:8888/api/S7/Update?id=${row.id}`, EditForm.value, {
        headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
    }).then((response) => {
